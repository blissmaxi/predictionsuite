...
import type { UnifiedEvent, UnifiedMarket } from '../types/unified.js';
...
function printMarket(market: UnifiedMarket, indent = '  ') {
...
function printEvent(event: UnifiedEvent) {
  console.log(`\n[Event] ${event.title}`);
...
function validateMarket(market: UnifiedMarket): string[] {
  const issues: string[] = [];
  ...
  if (!market.question) issues.push('Missing question');
  if (!market.eventId) issues.push('Missing eventId');
  if (!market.sourceUrl) issues.push('Missing sourceUrl');
