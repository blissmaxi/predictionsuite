...
async function fetchEvents(limit = 20, status = 'open'): Promise<KalshiEvent[]> {
...
async function fetchEventWithMarkets(eventTicker: string): Promise<KalshiEventWithMarkets> {
...
async function fetchMarkets(limit = 20, status = 'open'): Promise<KalshiMarket[]> {
  const params = new URLSearchParams({
    limit: limit.toString(),
    status,
  });
...
async function fetchMarket(ticker: string): Promise<KalshiMarket> {
...
async function fetchOrderBook(marketTicker: string): Promise<KalshiOrderBook> {
...
async function fetchSeries(limit = 10): Promise<KalshiSeries[]> {
  const params = new URLSearchParams({
    limit: limit.toString(),
  });
...
function summarizeMarket(market: KalshiMarket): KalshiMarketSummary {
  return {
...
function summarizeEvent(event: KalshiEvent, markets: KalshiMarket[]): KalshiEventSummary {
  return {
...
function printEventSummary(summary: KalshiEventSummary) {
  console.log(`\n[Event] ${summary.title}`);
...
function printOrderBook(orderBook: KalshiOrderBook, marketTicker: string) {
