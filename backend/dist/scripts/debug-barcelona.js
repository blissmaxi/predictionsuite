...
async function main() {
    ...
    const polyBarcelona = polyEvent.markets.find((m) => m.question?.toLowerCase().includes('barcelona'));
    ...
    // Kalshi
    const kalshiResp = await fetch(`${KALSHI_API_URL}/events?limit=100&with_nested_markets=true&status=open&series_ticker=KXUCL`);
    const kalshiData = await kalshiResp.json();
    const kalshiEvent = kalshiData.events.find((e) => e.event_ticker === 'KXUCL-26');
    const kalshiBarcelona = kalshiEvent.markets.find((m) => m.yes_sub_title?.toLowerCase().includes('barcelona'));
    console.log('\nKalshi Barcelona:');
