...
// ============ Polymarket ============
async function fetchPolymarketMarkets(slug) {
...
// ============ Kalshi ============
async function fetchKalshiMarkets(seriesTicker, eventTicker) {
    console.log(`\nFetching Kalshi: Series ${seriesTicker}, Event ${eventTicker}`);
    const response = await fetch(`${KALSHI_API_URL}/events?limit=100&with_nested_markets=true&status=open&series_ticker=${seriesTicker}`);
    const data = await response.json();
...
async function main() {
    ...
    const polyBarcelona = polyMarkets.find(m => m.question.toLowerCase().includes('barcelona'));
    const kalshiBarcelona = kalshiMarkets.find(m => m.question.toLowerCase().includes('barcelona'));
    console.log('\n--- Raw Market Data ---');
    ...
    // Step 3: Run matching
    console.log('\n--- Market Matching ---');
    const pairs = matchMarketsWithinEvent(polyMarkets, kalshiMarkets, 'sports', 'UEFA Champions League');
    console.log(`Total pairs matched: ${pairs.length}`);
    // Find Barcelona pair
    const barcelonaPair = pairs.find(p => p.matchedEntity.toLowerCase().includes('barcelona'));
    if (barcelonaPair) {
        console.log('\nBarcelona Pair:');
    ...
    const polySpurs = polyNBA.find(m => m.question.toLowerCase().includes('san antonio') ||
        m.question.toLowerCase().includes('spurs'));
    const kalshiSpurs = kalshiNBA.find(m => m.question.toLowerCase().includes('san antonio'));
    console.log('\n--- Raw Market Data ---');
    ...
    const nbaPairs = matchMarketsWithinEvent(polyNBA, kalshiNBA, 'sports', 'NBA Champion');
    const spursPair = nbaPairs.find(p => p.matchedEntity.toLowerCase().includes('san antonio'));
    if (spursPair) {
        console.log('\n--- Matched Pair ---');
