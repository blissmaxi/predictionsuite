...
async function fetchMarkets(limit = 20, active = true) {
    const params = new URLSearchParams({
        limit: limit.toString(),
        active: active.toString(),
        closed: 'false',
...
function parseJsonString(jsonStr, fallback) {
...
function summarizeMarket(market) {
    const prices = parseJsonString(market.outcomePrices, ['0', '0']);
    const tokenIds = parseJsonString(market.clobTokenIds, []);
    return {
        id: market.id,
...
async function main() {
    ...
    try {
        ...
        const tokenIds = parseJsonString(markets[0]?.clobTokenIds, []);
