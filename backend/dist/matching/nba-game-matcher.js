 ...
 */
export function getTeamFromCode(code) {
 ...
 */
export function parsePolymarketNbaSlug(slug) {
 ...
 */
export function isNbaGameSlug(slug) {
 ...
 */
export function generateKalshiNbaTicker(date, awayCode, homeCode) {
 ...
 */
export function generatePolymarketNbaSlug(date, awayCode, homeCode) {
 ...
 */
function parseKalshiNbaTicker(ticker) {
 ...
 */
export async function fetchNbaGameMatches(startDate, endDate) {
    const matches = [];
    try {
        // Fetch open NBA game events from Kalshi
        const response = await fetch(`https://api.elections.kalshi.com/trade-api/v2/events?series_ticker=${KALSHI_SERIES}&limit=100&status=open`);
        if (!response.ok) {
        ...
        for (const event of events) {
            ...
            matches.push({
                ...
                polymarketSlug: generatePolymarketNbaSlug(gameDate, parsed.awayCode, parsed.homeCode),
                kalshiTicker: ticker,
 ...
 */
export function formatGameMatch(match) {
