 ...
 */
export async function fetchPolymarketOrderBook(yesTokenId, noTokenId) {
 ...
 */
async function fetchPolymarketSingleBook(tokenId) {
    const emptyBook = { bids: [], asks: [] };
    try {
        const url = `${POLYMARKET.CLOB_API_URL}/book?token_id=${tokenId}`;
        const response = await fetch(url);
        if (!response.ok) {
            throw new ApiError(`Failed to fetch order book`, 'polymarket', response.status, { tokenId });
        }
...
export async function fetchKalshiOrderBook(ticker) {
    ...
    try {
        ...
        const response = await fetch(url);
        if (!response.ok) {
            throw new ApiError(`Failed to fetch order book`, 'kalshi', response.status, { ticker });
        }
        const data = (await response.json());
 ...
 */
function parseOrderBookLevels(levels, sortOrder) {
    if (!levels)
        return [];
    const parsed = levels
        .map((level) => ({
 ...
 */
function parseKalshiLevels(levels, priceMode, sortOrder) {
    const parsed = levels
        .map(([priceStr, qty]) => {
 ...
 */
function createEmptyOrderBook(platform, marketId) {
    return {
        platform,
        marketId,
        yesBids: [],
 ...
 */
export function hasLiquidity(book) {
    return (book.yesBids.length > 0 ||
 ...
 */
export function getBestPrices(book) {
    return {
 ...
 */
export function getDepth(book) {
    const sumSize = (levels) => levels.reduce((sum, l) => sum + l.size, 0);
    return {
 ...
 */
export function getSpread(book, outcome) {
